...
typedef struct para{
    int samples_per_pixel, image_width, image_height, y_s, y_e;
    ray camera;
    vec3 cx, cy;
    vec3 *image;
} Para;
...
void* loop(void *para){
    Para *arg = (Para*)para;
    int s = 0, x = 0, i = 0, sy = 0, sx = 0, y = 0;
    for (y = arg->y_s; y < arg->y_e; ++y){
        unsigned short xi[3] = { 0, 0 }; 
        xi[2] = y * y * y;
        for (x = 0; x < arg->image_width; ++x) {
            i = (arg->image_height - y - 1) * arg->image_width + x;
            for (sy = 0; sy < 2; ++sy) {
                for (sx = 0; sx < 2; ++sx) {
                    vec3 accumulated_color = vec3_init(0, 0, 0);
                    for (s = 0; s < arg->samples_per_pixel; s++) {
                        /* Randomly generate a ray from camera */
                        ray camera_ray = generate_ray(&(arg->camera), xi, arg->image_width, arg->image_height, 
                                                        x, y, &(arg->cx), &(arg->cy), sx, sy);
                        ...
                        accumulated_color = vec3_add(accumulated_color, vec3_mulf(camera_ray_color, (1. / arg->samples_per_pixel)));
                    }
                    /* Add `accumulated_color` for this subpixel to the color for the image pixel, which can be imagined as 1 = 0.25 * 4 */
                    arg->image[i] = vec3_add(arg->image[i], vec3_mulf(vec3_init(clamp(accumulated_color.x), clamp(accumulated_color.y), clamp(accumulated_color.z)), .25));
                }
            }
        }
    }
...
int main(int argc, char *argv[]) {
    Para *arg_1 = malloc(sizeof(Para));
    Para *arg_2 = malloc(sizeof(Para));
    Para *arg_3 = malloc(sizeof(Para));
    Para *arg_4 = malloc(sizeof(Para));
    /* Create the thread */
    pthread_t t1, t2, t3, t4;
    vec3 *image;
    /* Image settings */
    arg_1->image_width = arg_2->image_width = arg_3->image_width = arg_4->image_width = 1024;
    arg_1->image_height = arg_2->image_height = arg_3->image_height = arg_4->image_height = 768;
    arg_1->samples_per_pixel = arg_2->samples_per_pixel = arg_3->samples_per_pixel = arg_4->samples_per_pixel = argc == 2 ? atoi(argv[1]) / 4 : 1;
    image = malloc(sizeof(vec3) * arg_1->image_width * arg_1->image_height);

    /* Camera settings */
    arg_1->camera = arg_2->camera = arg_3->camera = arg_4->camera = ray_init(vec3_init(50, 52, 295.6), vec3_unit(vec3_init(0, -0.042612, -1)));
    arg_1->cx = arg_2->cx = arg_3->cx = arg_4->cx = vec3_init(arg_1->image_width * .5135 / arg_1->image_height, 0, 0);
    arg_1->cy = arg_2->cy = arg_3->cy = arg_4->cy = vec3_mulf(vec3_unit(vec3_cross(arg_1->cx, arg_1->camera.d)), .5135);

    /* Ray tracing */
    memset(image, 0, sizeof(vec3) * arg_1->image_width * arg_1->image_height);
    arg_1->image = arg_2->image = arg_3->image = arg_4->image =image;
    arg_1->y_s = 0, arg_1->y_e = arg_1->image_height / 4;
    pthread_create(&t1, NULL, loop, arg_1);
    arg_2->y_s = arg_2->image_height / 4, arg_2->y_e = arg_2->image_height / 2;
    pthread_create(&t2, NULL, loop, arg_2);
    arg_3->y_s = arg_3->image_height / 2, arg_3->y_e = 3 * arg_3->image_height / 4;
    pthread_create(&t3, NULL, loop, arg_3);
    arg_4->y_s = 3 * arg_4->image_height / 4, arg_4->y_e = arg_4->image_height;
    pthread_create(&t4, NULL, loop, arg_4);
    ...
    write_image("image.ppm", image, arg_1->image_width, arg_1->image_height);
    free(arg_1), free(arg_2), free(arg_3), free(arg_4);
    free(image);
    return 0;
}