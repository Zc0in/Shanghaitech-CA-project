...
v_add_optimized_adjacent (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    ...
    for (int i = a; i < ARRAY_SIZE; i += omp_get_num_threads()){
      z[i] = x[i] + y[i];
    }
  }
}
...
v_add_optimized_chunks (double *x, double *y, double *z)
{
  ...
  {
    int a = omp_get_thread_num();
    int begin = a * (ARRAY_SIZE / omp_get_num_threads());
    int end = a == omp_get_num_threads() - 1 ? ARRAY_SIZE : (a + 1) * (ARRAY_SIZE / omp_get_num_threads());
